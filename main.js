...
var require_pubsub_ts = __commonJS({
  "node_modules/pubsub-ts/index.js"(exports) {
    "use strict";
    ...
    (function(PubSub3) {
      ...
      (function(NotificationType2) {
        NotificationType2[NotificationType2["standard"] = 0] = "standard";
        NotificationType2[NotificationType2["priority"] = 1] = "priority";
        NotificationType2[NotificationType2["urgent"] = 2] = "urgent";
      })(NotificationType = PubSub3.NotificationType || (PubSub3.NotificationType = {}));
      var Subscriber = (
        /** @class */
        function() {
          function Subscriber2(key) {
            if (key === void 0) {
          ...
          Subscriber2.prototype.on = function(eventName, callback) {
            this.interests.set(eventName, callback);
          };
          Subscriber2.prototype.off = function(eventName) {
            this.interests.delete(eventName);
          };
          Subscriber2.prototype.getKey = function() {
            return this.key;
          };
          Subscriber2.prototype.start = function() {
            this.shouldPost = true;
            this.processNotifications();
          };
          Subscriber2.prototype.pause = function() {
            this.shouldPost = false;
          };
          Subscriber2.prototype.post = function(notification) {
          ...
          Subscriber2.prototype.processNotifications = function() {
            if (!this.shouldPost) {
              return;
            }
          ...
          Subscriber2.prototype.postNotifications = function(queue) {
            var i = queue.length;
            while (i--) {
          ...
          Subscriber2.prototype.postCallback = function(notification) {
      ...
      var Publisher = (
        ...
        function() {
          ...
          Publisher2.prototype.add = function(subscriber) {
            var key = subscriber.getKey();
            if (!this.has(key)) {
              this.subscribers.set(key, subscriber);
            }
          };
          Publisher2.prototype.delete = function(subscriber) {
            this.subscribers.delete(subscriber.getKey());
          };
          Publisher2.prototype.has = function(key) {
            return this.subscribers.has(key);
          };
          Publisher2.prototype.notify = function(eventName, data) {
            this.sendNotification(eventName, data, NotificationType.standard);
          };
          Publisher2.prototype.notifyPriority = function(eventName, data) {
            this.sendNotification(eventName, data, NotificationType.priority);
          };
          Publisher2.prototype.notifyUrgent = function(eventName, data) {
            this.sendNotification(eventName, data, NotificationType.urgent);
          };
          Publisher2.prototype.sendNotification = function(eventName, data, type) {
      ...
      PubSub3.Publisher = Publisher;
    })(PubSub2 = exports.PubSub || (exports.PubSub = {}));
  }
});
...
var prefix = id;
var ENCRYPT_FILE_GLYPH = "vault";
var NEW_KEY_GLYPH = "install";
var VIEW_TYPE_ENCRYPTED_EDITOR = `${prefix}-encrypted-editor`;
var ENCRYPTION_SERVICE_EVENTS = `${prefix}-encryption-service-events`;
var KEY_LIST_UPDATE_EVENT = `${prefix}-key-list-updated`;
var AUTH_REQUESTED_EVENT = `${prefix}-key-access-request`;
var KEY_CREATED_EVENT = `${prefix}-key-created`;
var KEY_DELETED_EVENT = `${prefix}-key-deleted`;

// src/views/FileEditorView.tsx
var FileEditorView = class extends import_obsidian2.FileView {
  getViewType() {
    return VIEW_TYPE_ENCRYPTED_EDITOR;
  }
  constructor(leaf) {
    super(leaf);
  }
...
// src/views/KeyGeneratorModal.tsx
var import_obsidian3 = require("obsidian");
var import_lodash = __toESM(require_lodash());
var DEFAULT_SETTINGS2 = {
  passphrase: ""
};
var KeyGeneratorModal = class extends import_obsidian3.Modal {
  constructor(app, settings, cb) {
    super(app);
    this.cb = cb;
    this.keyRequest = { ...DEFAULT_SETTINGS2 };
    this.settings = settings;
  }
  submitRequest() {
    if (this.keyRequest.passphrase === "") {
      new import_obsidian3.Notice("Passphrase is required and cannot be an empty string");
      return false;
    } else if (this.keyRequest.passphrase.length > 64) {
      new import_obsidian3.Notice("Passphrase character limit arbitrarily set to 64");
      return false;
    }
    try {
      ...
      console.debug(err);
      new import_obsidian3.Notice("Something didn't work...");
      return false;
    }
    return true;
  }
  onOpen() {
    ...
    new import_obsidian3.Setting(contentEl).setName("Passphrase for encryption key").addText((text) => text.onChange((value) => {
      (0, import_lodash.set)(this.keyRequest, "passphrase", value);
    }));
    new import_obsidian3.Setting(contentEl).setName("Friendly name").addText((text) => {
      text.setPlaceholder("Key Name");
      text.onChange((value) => {
        (0, import_lodash.set)(this.keyRequest, "name", value);
      });
    });
    new import_obsidian3.Setting(contentEl).addButton((button) => button.setButtonText("Create").onClick((evt) => {
      this.submitRequest() && this.close();
    })).addButton((button) => button.setButtonText("Cancel").onClick((evt) => {
      this.close();
    }));
  }
  onClose() {
...
// src/views/KeyAuthorizorModal.tsx
var import_obsidian4 = require("obsidian");
var KeyAuthorizerModal = class extends import_obsidian4.Modal {
  constructor(app, key, cb) {
    super(app);
    this.cb = cb;
    this.key = key;
  }
  submitRequest() {
    if (this.passphrase === "") {
      new import_obsidian4.Notice("Passphrase is required and cannot be an empty string");
      return false;
    }
  ...
  onOpen() {
    ...
    new import_obsidian4.Setting(contentEl).setName("Passphrase").addText((text) => text.onChange((value) => {
      this.passphrase = value;
    }));
    new import_obsidian4.Setting(contentEl).addButton((button) => button.setButtonText("Authorize").onClick((evt) => {
      this.submitRequest() && this.close();
    })).addButton((button) => button.setButtonText("Cancel").onClick((evt) => {
      this.close();
    }));
...
var EncryptionKeySuggest = class extends import_obsidian5.EditorSuggest {
  constructor(app, settings, subscription) {
    super(app);
    ...
    this.subscription.on(KEY_LIST_UPDATE_EVENT, (evt) => {
...
` : `${this.FM_KEY}: ${value}`;
      ...
      this.editor.setCursor(
        this.lineNumber === this.editor.lineCount() ? line.length - 1 : { line: this.lineNumber + 1, ch: 0 }
      );
      this.reset();
    }
  }
...
// src/encryption/EncryptionService.tsx
var import_lodash2 = __toESM(require_lodash());
var import_obsidian6 = require("obsidian");
var crypto = __toESM(require("crypto"));
var import_pubsub_ts = __toESM(require_pubsub_ts());
var service;
var alg = "aes-256-cbc";
var Authorization = class {
  constructor(kid, secret, ttl) {
    let expiration = ttl && ttl > 0 ? (0, import_obsidian6.moment)().add(import_obsidian6.moment.duration(ttl, "second")) : null;
    this.kid = kid;
    this.secret = secret;
    this.isValid = () => {
      return !expiration || expiration.isAfter((0, import_obsidian6.moment)());
    };
    this.renew = () => {
      expiration = !expiration ? null : (0, import_obsidian6.moment)().add(import_obsidian6.moment.duration(ttl, "second"));
    };
  }
};
var encryptionService = class {
  constructor() {
    this.authList = /* @__PURE__ */ new Map();
    this.eventsQueue = new import_pubsub_ts.PubSub.Publisher();
  }
  subscribe() {
    const sub = new import_pubsub_ts.PubSub.Subscriber();
    this.eventsQueue.add(sub);
    return sub;
  }
  authorizeKey(keyId) {
    var _a;
    this.eventsQueue.notify(AUTH_REQUESTED_EVENT, { keyId });
    if (this.authList.has(keyId)) {
      if (((_a = this.authList.get(keyId)) == null ? void 0 : _a.isValid()) || false) {
        return Promise.resolve(true);
      }
      this.authList.delete(keyId);
    }
    const result = new Promise((resolve) => {
      const ketSey = this.settings.Keys[keyId];
      if (ketSey) {
  ...
  xcrypt(keySet, passphrase, bData, action) {
  ...
  lookupPassphrase(kid) {
    if (!this.authList.has(kid)) {
      return "";
    }
  ...
  createKey() {
    const result = new Promise((resolve) => {
      new KeyGeneratorModal(this.app, this.settings, (req) => {
        ...
        this.saveData(this.settings).then(() => {
          this.eventsQueue.notify(KEY_LIST_UPDATE_EVENT, {
            keyId: keySet.id,
            keys: (0, import_lodash2.cloneDeep)(this.settings.Keys)
          });
          this.eventsQueue.notify(KEY_CREATED_EVENT, { keyId: keySet.id });
        });
        resolve(keySet.id);
      }).open();
    });
  ...
  generatePrivateKey(req) {
    const { passphrase, hint, name: name2, idleTTL } = req;
...
function getService(parent, settings) {
  if (!service) {
    if (parent) {
      const { app } = parent;
      service = new encryptionService();
      (0, import_lodash2.extend)(service, {
...
// src/main.tsx
var frontmatterMatcher = /---\s*[\s\S]*?\s*---/g;
var FileEncryptPlugin = class extends import_obsidian7.Plugin {
  constructor(app, manifest) {
    super(app, manifest);
  }
  async onload() {
    await this.loadSettings();
    this.encryptionService = getService(this, this.settings);
    ...
    this.registerView(VIEW_TYPE_ENCRYPTED_EDITOR, (leaf) => new FileEditorView(leaf));
    ...
    const fnCreateNewKey = () => this.encryptionService.createKey().then((kid) => new import_obsidian7.Notice(`Created key with id "${kid}"`));
    this.addRibbonIcon(
      NEW_KEY_GLYPH,
      "New Encryption Key",
      fnCreateNewKey
...
`);
        ...
        try {
          ...
          console.debug(err);
          new import_obsidian7.Notice("Well, this is't right.");
        }
      });
    });
  }
  ...
  async loadSettings() {
    this.settings = (0, import_lodash3.extend)({}, DEFAULT_SETTINGS, await this.loadData());
  }
  saveSettings() {
